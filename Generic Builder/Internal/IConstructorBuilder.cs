...
namespace InterestingCodeCollection.GenericBuilder.Internal
{
    ...
    {
        /// <summary>
        /// Use this method within an implementation of <see cref="BuilderRegistration"/> to use values set using any of the <see cref="M:With"/> overloads within the constructor expression.
        /// </summary>
        ...
        [PublicAPI]
        T2 From<T2>(Expression<Func<T, T2>> expression, IBuilder<T2> defaultValueBuilder);
        ...
        [PublicAPI]
        T2 From<T2>(Expression<Func<T, T2>> expression, T2 defaultValue);
    }
}
